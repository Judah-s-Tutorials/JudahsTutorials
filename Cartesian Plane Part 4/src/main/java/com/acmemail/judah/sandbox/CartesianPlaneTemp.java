...
public class CartesianPlaneTemp extends JPanel
{
    ...
    //////////////////////////////////////////////////
    // 
    // Temporary stuff to support a test ploat.
    //
    //////////////////////////////////////////////////
    private final List<Point2D> plot    = new ArrayList<>();
    public void addPlot( List<Point2D> plot )
    {
        this.plot.clear();
        this.plot.addAll( plot );
    }
    private void drawPlot()
    {
        double      centerXco   = gridRect.getCenterX();
        double      centerYco   = gridRect.getCenterY();
        Ellipse2D   dot     = new Ellipse2D.Double();
        gtx.setColor( Color.black );
        for ( Point2D point : plot )
        {
            double  xco = centerXco + point.getX() * gridUnit;
            double  yco =  centerYco - point.getY() * gridUnit;
            dot.setFrame( xco, yco, 3, 3 );
            gtx.fill( dot );
        }
    }
    ...
    public void paintComponent( Graphics graphics )
    {
        ...
        drawVerticalLabels();
        paintMargins();
        drawPlot(); // temporary, just for fun
    }
