 ...
 * @author Jack Straub
 */
public class CommandExecutor_copy
{
     ...
     *              may not  be null;
     */
    public CommandExecutor_copy( CartesianPlane plane )
    {
        if ( plane == null )
    ...
    /**
     * Select the equation with the given name from the equation map
     * from the equation map. If <em>name</em> is empty
     * the operator will be prompted for a name.
     * If an equation is successfully selected, 
     * a new InputParser will be created
     * containing the equation.
     * If an equation is not selected
    ...
    {
        Equation    equation    = EquationMap.getEquation( name );
        if ( equation != null )
            inputParser = new InputParser( equation );
    }
